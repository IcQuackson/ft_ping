...
void ft_ping(t_arguments *arguments)
{
	...
	resolve(arguments->host, echo_request.ip_host, echo_request.dns_host);
	strncpy(echo_request.user_input, arguments->host, NI_MAXHOST);
...
void send_icmp_request(int sockfd, t_echo_request *echo_request)
{
	...
	{
		printf("PING %s(%s (%s)) %ld(%ld) bytes of data.\n", echo_request->user_input, echo_request->dns_host, echo_request->ip_host, sizeof(icmphdr.icmp_data), sizeof(icmphdr));
	}
